...
const configSchema = Joi.object({
    ...
    broker: Joi.string()
        .default('in-memory')
        .valid('in-memory', 'redis')
        .description('backend broker to use'),
    brokerWildcard: Joi.string()
        .default('%')
        .valid('%', '*')
    ...
    retries: Joi.number()
        .min(1)
        .default(5)
        .description('How often a handler should be tried before giving up'),
    concurrency: Joi.number()
        .min(1)
        .default(1)
        .description('How many handler are allowed to be executed in parallel'),
    topicTypes: Joi.array()
        .items(
            Joi.object({
                topic: Joi.string()
                    .description('name of the topic this tyope applies to')
                    .required(),
                type: Joi.object().description('the compiled AVRO type as returned by (avsc.parse(<schema>))'),
            })
        )
        .default({})
        .description('topic/types map'),
    requestEventBuffer: Joi.object({
        enabled: Joi.boolean()
            .default(false)
            .description('whether the eventBuffer is enabled'),
        resIdAccessor: Joi.func().default(evtData => (evtData.payload ? evtData.payload.resId : null)),
    })
        .default({enabled: false})
        .description(
            'Buffer all events in the current request context deduplicate and send after the handler finshed. Only used for event created using "publishHttpReply" method.'
        ), //eslint-disable-line
});
...
class AtrixPubsub {
    constructor(atrix, service, config) {
        ...
        if (this.config.requestEventBuffer && this.config.requestEventBuffer.enabled) {
            ...
            httpEndpoint.instance.server.ext('onPreResponse', async (request, h) => {
                await eventBuffer.publishEvents(request, this);
                return h.continue;
            });
        }
    }
    ...
    async setupBroker() {
        ...
        switch (this.config.broker) {
            case 'redis': {
                this.redisClient = redis(this.config.redis);
                await waitForRedis(this.redisClient, this.log);
    ...
    async start() {
        ...
        this.service.events.on('stopping', async () => {
            if (this.redisClient) {
                this.log.info('Closing Redis connection ...');
    ...
    async stop() {
        if (this.mq) {
            this.log.info('Closing MQEmitter ...');
            this.mq.close(() => {});
            this.log.info('Closesd MQEmitter');
        }
    }
    ...
    async publishHttpReply(req, data, status, routePrefix) {
        ...
        if (
            this.config.requestEventBuffer &&
            this.config.requestEventBuffer.enabled &&
            req.headers['x-atrix-context-req-id']
        ) {
    ...
    resolveHandlers(broker) {
        ...
        return handlerFiles
            .filter(file => file.indexOf('specs.js') === -1 && file.indexOf('.js') === file.length - 3)
            .map(file => {
                const module = require(file); // eslint-disable-line
                const name = AtrixPubsub.fixupTopicName(
                    file
                        .replace(this.config.handlerDir, '')
                        .replace(/\^/g, '/')
                        .replace(/_\.js|\.js/, '')
                );
