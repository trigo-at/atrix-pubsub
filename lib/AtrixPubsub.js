...
class AtrixPubsub {
	...
	subscribe(topic, handler) {
		return this.ascoltatore.subscribeAsync(topic, handler);
	}

	publish(topic, data) {
		this.log.debug(`publish on topic: "${topic}"`, data);
		const entry = R.find(R.propEq('topic', topic), this.config.topicTypes);
		if (entry) {
			...
			this.log.warn(`No type sepcified for topic: ${topic}. Sending untyped message!`);
		}

		return this.ascoltatore.publishAsync(topic, data); // eslint-disable-line no-multi-assign
	}


	async publishHttpReply(req, data, status) {
		const topic = `${this.service.name.toLowerCase()}.svc${req.route.path.replace(/[{}]/g, '_')}/${req.method}.${statusName(status)}`;
	...
	resolveHandlers(broker) {
		...
		return handlerFiles.filter((file) => { // eslint-disable-line
			...
			let name = file.replace(this.config.handlerDir, '').replace(/\^/g, '/').replace(/_\.js|\.js/, '');
			if (name[0] === '/') {
				name = name.substr(1);
			}
